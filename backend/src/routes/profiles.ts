...
// Reverse-geocodes precise (pre-jitter) coordinates into a city name via
// OpenStreetMap Nominatim. Never throws - resolves to null on any failure,
// timeout, or missing address component, so it can never block a profile
// save. Nominatim usage policy requires a descriptive User-Agent and no
// aggressive request rate; this is called at most once per profile update.
  ...
  try {
    ...
    const response = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}&zoom=10&addressdetails=1`,
      {
        ...
        headers: {
          "User-Agent": "BuddiesPride/1.0 (jobber-match profile location)",
          "Accept-Language": "en",
        },
      }
    );
...
const updateSchema = z.object({
  ...
  orientationVisible: z.boolean().nullable().optional(),
  genderIdentity: z.string().nullable().optional(),
  genderVisible: z.boolean().nullable().optional(),
...
router.patch("/me", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (fields.latitude != null && fields.longitude != null) {
    // Auto-detect city from precise (pre-jitter) coordinates, but only
    // when the caller didn't explicitly set city themselves - a manual
    // edit on the profile page should never be overwritten by this.
    if (fields.city === undefined) {
      const detectedCity = await reverseGeocodeCity(fields.latitude, fields.longitude);
      if (detectedCity) {
  ...
  const columnMap: Record<string, string> = {
    ...
    orientationVisible: "orientation_visible",
    genderIdentity: "gender_identity",
    genderVisible: "gender_identity_visible",
    pronouns: "pronouns",
    photos: "photos",
    latitude: "latitude",
    longitude: "longitude",
